...
def sync_from(url: str = typer.Argument(None, help="the main server, e.g. http://macm5.local (default: mirror_of from the config)"),
    ...
    """Pull a backup from another Recipe Library and merge it into this one (backup server)."""
    from .db.engine import init_engine
    from .db.migrate import migrate
    from .mirror import sync_once
    cfg = get_settings()
    cfg.ensure_dirs()
    migrate(cfg.db_path)
    init_engine(cfg.db_path)
    try:
        typer.echo(sync_once(url, "replace" if replace else "merge"))
    except Exception as e:  # noqa: BLE001
        typer.echo(f"!! {type(e).__name__}: {e}")
        raise typer.Exit(code=1)
